...
void OGLRenderer::SetShaderLight(Light& l) {
	...
	location = glGetUniformLocation(currentShader->GetProgram(), "lightDirection");
	glUniform3fv(location, 1, (float*)&l.GetDirection());
	glUniform1f(glGetUniformLocation(currentShader->GetProgram(), "cutAngleCos"), l.GetSpotLightCutAngle());
}
